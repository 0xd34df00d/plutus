...
pAOpts = AOpts
    ...
    <*> option auto ( metavar "N"
                    <> long "top"
                    <> short 't'
                    <> help "Output the top N variables sorted by decreasing retained size"
                    <> value 50
                    <> showDefault
                    )
    <*> output
...
pPirOpts = hsubparser $
    command "analyse"
        (info (Analyse <$> pAOpts) $
            progDesc "Given a PIR flat as input, deserialise and analyse the program looking for variables with the largest retained size.")
  <> command "compile"
        (info (Compile <$> pCOpts) $
            progDesc "Given a PIR flat, deserialize it and test if it can be successfully compiled to PLC.")
...
loadPirAndAnalyse aopts = do
    ...
    let
        ...
        -- sort the map by decreasing retained size and take the largest N entries
        topRetained = take (fromIntegral $ aTopN aopts) . sortOn (negate . snd) $ IM.assocs retentionMap

        -- change uniques to texts and use csv-outputtable records
        topRecords :: [RetentionRecord]
        topRecords = (\(i,s) -> RetentionRecord (IM.findWithDefault "???" i nameTable) i s) <$> topRetained

    -- encode to csv and output it
    Csv.encodeDefaultOrderedByName topRecords &
        case aOut aopts of
